...
void Application::processWindowEvents() {
	...
	while (_window.pollEvent(event)) {
		if (event.type == sf::Event::Closed)
			...
			if (_state == Play && event.key.code == sf::Keyboard::R)
				_board.clear();
			if ((_state == Init || _state == GameOver) && event.key.code == sf::Keyboard::Enter)
				_state = Play;
